...
def _fill_internal_voids(
    foreground: np.ndarray,
    refined_alpha: np.ndarray,
    base_mask: np.ndarray,
    min_hole_area: float = 18.0,
    inpaint_radius: int = 3,
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Detects enclosed low-alpha regions inside the garment silhouette and fills them with
    inpainted RGB plus reinforced alpha to avoid see-through artifacts.
    """
    if foreground.size == 0 or not np.any(base_mask):
        return foreground, refined_alpha

    mask_u8 = (base_mask.astype(bool).astype(np.uint8)) * 255
    contours, hierarchy = cv2.findContours(mask_u8, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
    if hierarchy is None:
        return foreground, refined_alpha

    hole_mask = np.zeros_like(mask_u8)
    for idx, hier in enumerate(hierarchy[0]):
        parent = hier[3]
        if parent != -1:
            area = cv2.contourArea(contours[idx])
            if area >= min_hole_area:
                cv2.drawContours(hole_mask, contours, idx, 255, thickness=-1)

    if not np.any(hole_mask):
        return foreground, refined_alpha

    filled_foreground = cv2.inpaint(foreground, hole_mask, inpaint_radius, cv2.INPAINT_TELEA)

    alpha_u8 = np.clip(refined_alpha * 255.0, 0.0, 255.0).astype(np.uint8)
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    dilated = cv2.dilate(alpha_u8, kernel, iterations=2)
    blurred = cv2.GaussianBlur(dilated.astype(np.float32), (0, 0), sigmaX=1.2, sigmaY=1.2)
    blurred = np.clip(blurred, 0.0, 255.0).astype(np.uint8)

    hole_idx = hole_mask.astype(bool)
    alpha_u8 = alpha_u8.copy()
    alpha_u8[hole_idx] = np.maximum(alpha_u8[hole_idx], blurred[hole_idx])
    alpha_u8[hole_idx] = np.clip(alpha_u8[hole_idx], 190, 255)
    filled_alpha = np.clip(alpha_u8.astype(np.float32) / 255.0, 0.0, 1.0)
    support = cv2.dilate(base_mask.astype(np.uint8), kernel, iterations=1)
    support = (support > 0).astype(np.float32)
    filled_alpha *= support

    return filled_foreground, filled_alpha
...
def _artifact_aware_overlay(raw_img: np.ndarray, raw_target: np.ndarray, raw_alpha: np.ndarray) -> np.ndarray:
    ...
    cleaned_target, refined_alpha = _fill_internal_voids(cleaned_target, refined_alpha, base_mask)
